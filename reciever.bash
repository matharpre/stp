...
while [[ "$exitloop" != "true" ]]
do
echo "INFO: Currently searching timestamp ranging from $start to $finish (s)"
...
echo $starttime1 $starttime2 $starttime3 $starttime4
...
echo "INFO: Extracting data inside the interval $starttime1 - $end (s)"
while [[ "$complete" != true ]]
do
...
echo "INFO: Converting output file to original form"
cat $outputfile | od -A n -t x1 | tr -d ' \n' > "$outputfile"1
xxd -r -p "$outputfile"1 > $outputfile
rm "$outputfile"1
